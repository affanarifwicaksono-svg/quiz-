...
// FUNGSI LAYAR PEMBUKA
function setGender(g) {
    playerGender = g;
    document.getElementById('btn-bapak').classList.toggle('active', g === 'Bapak');
    document.getElementById('btn-ibu').classList.toggle('active', g === 'Ibu');
    checkReady();
}
...
function checkReady() {
    ...
    btn.disabled = !(playerGender !== "" && name !== "");
}

function startGame() {
    playerName = document.getElementById('player-name-input').value.trim();
    document.getElementById('player-title').innerText = playerGender;
...
const scenarios = [
    {
        t: "Awal Ramadhan: Muhammadiyah Hisab, NU Rukyatul Hilal. Warga mulai berdebat.",
        a: ["Saling klaim paling benar", "Ajak saling hormat & ijtihad sah", "Ikuti selera pribadi"],
        correct: 1, icon: "🌙", desc: "HILAL & HISAB"
    },
    {
        ...
        correct: 0, icon: "🕌", desc: "SHALAT TARAWIH"
    },
    {
    ...
    },
    {
        ...
        correct: 1, icon: "🎒", desc: "ANAK SEKOLAH"
    },
    {
        ...
        correct: 0, icon: "📦", desc: "BANTUAN SOSIAL"
    },
    {
        ...
        correct: 1, icon: "💰", desc: "KOTAK AMAL"
    },
    {
        t: "Gaya Dakwah: Ustadz baru sering menyindir amalan warga di mimbar masjid.",
        a: ["Biarkan bebas bicara", "Beri teguran halus agar dakwah sejuk", "Minta warga boikot"],
        correct: 1, icon: "🎤", desc: "MIMBAR DAKWAH"
    },
    {
        t: "Pemilihan Pemimpin: Warga pecah antara kandidat Muhammadiyah dan NU.",
        a: ["Tekankan kualitas kerja, bukan organisasi", "Memihak salah satu", "Batalkan pemilihan"],
        correct: 0, icon: "🗳️", desc: "PEMILIHAN"
    },
    {
        t: "Kerja Bakti: Jadwal bentrok dengan acara pengajian besar warga.",
        a: ["Batalkan kerja bakti", "Atur ulang jadwal agar keduanya jalan", "Paksa warga pilih satu"],
        correct: 1, icon: "🧹", desc: "KERJA BAKTI"
    },
    {
        t: "Speaker Masjid: Debat volume suara dzikir yang dianggap terlalu keras.",
        a: ["Matikan total speaker", "Musyawarah batasan volume yang nyaman", "Biarkan sampai konflik"],
        correct: 1, icon: "📢", desc: "SPEAKER MASJID"
    }
];
...
function updateUI() {
    if(currentIdx < scenarios.length) {
        ...
        document.getElementById("q-text").innerText = `Ada masalah: ${s.t}`;
...
function handleChoice(p) {
    if(p === scenarios[currentIdx].correct) { 
        currentScore += 10; 
        alert("✅ Alhamdulillah! Adem tenan."); 
    } else { 
        currentScore -= 10; 
        alert("⚠️ Wah, warga mulai tegang."); 
    }
...
function showResult() {
    ...
    let rank = ""; let msg = "";
    if(currentScore >= 180) { rank = "MUJAHID UKHUWAH 🏆"; msg = `Luar biasa, ${playerGender} ${playerName}! Anda adalah Pemersatu Bangsa!`; }
    else if(currentScore >= 120) { rank = "PEMIMPIN MODERAT 🥈"; msg = `Bagus, ${playerGender} ${playerName}, kampung tetap rukun.`; }
    else { rank = "PERLU TABAYYUN ☕"; msg = `Ayo, ${playerGender} ${playerName}, lebih sering ajak warga ngopi bareng.`; }
